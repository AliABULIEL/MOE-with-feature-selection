"""
Dataset Evaluation for BH Routing Framework
============================================

This module provides dataset loading and evaluation functions for measuring
the quality impact of BH routing on standard NLP benchmarks.
...
Usage:
    from bh_routing_evaluation import evaluate_perplexity, evaluate_lambada

    # With internal logging
    result = evaluate_perplexity(
        model, tokenizer, dataset,
        patcher=patcher,  # BHRoutingIntegration instance
        max_samples=200
    )
...
def load_lambada(
    ...
    """
    try:
        dataset = load_dataset('lambada', split=split, trust_remote_code=True)
    except:
        # Fallback to different configuration if needed
        dataset = load_dataset('EleutherAI/lambada_openai', 'en', split=split, trust_remote_code=True)
...
def evaluate_perplexity(
    ...
    Args:
        ...
        patcher: Optional BHRoutingIntegration instance for logging
