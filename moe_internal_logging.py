...
class RouterLogger:
    """
    Captures internal router outputs using forward hooks on layer.mlp.gate.

    This provides visibility into what the router is doing at each layer,
    essential for understanding routing behavior of BH, HC, or TopK.
    """
    ...
    def register_hooks(self, top_k: int = 8):
        ...
        def create_hook(layer_idx):
            def hook_fn(module, input, output):
                ...
                try:
                    router_logits = output  # [batch*seq, num_experts]

                    # Compute softmax probabilities
                    probs = F.softmax(router_logits, dim=-1, dtype=torch.float32)

                    # Get top-k
                    topk_weights, topk_indices = torch.topk(probs, min(self.top_k, probs.shape[-1]), dim=-1)
...
class InternalRoutingLogger:
    ...
    def __init__(self, output_dir: str, experiment_name: str, routing_method: str = 'unknown'):
        """
    ...
    def get_aggregate_stats(self) -> Dict[str, Any]:
        ...
        for layer_idx, stats_list in self.per_layer_stats.items():
            layer_entropies[layer_idx] = float(np.mean([s['entropy'] for s in stats_list]))
            layer_concentrations[layer_idx] = float(np.mean([s['concentration'] for s in stats_list]))

        # Expert utilization
        total_activations = sum(self.expert_usage_counts.values())
        expert_distribution = {k: v / total_activations for k, v in self.expert_usage_counts.items()} if total_activations > 0 else {}
        ...
        return {
            ...
            'avg_loss': self.total_loss / self.total_tokens if self.total_tokens > 0 else float('inf'),
            'perplexity': float(np.exp(self.total_loss / self.total_tokens)) if self.total_tokens > 0 else float('inf'),
            ...
            'most_used_experts': sorted(self.expert_usage_counts.items(), key=lambda x: x[1], reverse=True)[:10],
            'least_used_experts': sorted(self.expert_usage_counts.items(), key=lambda x: x[1])[:10],
            **expert_count_stats
        }
        ...
        output = {
            ...
            'per_layer_summary': {
                layer: {
                    ...
                    'avg_concentration': float(np.mean([s['concentration'] for s in stats]))
                }
