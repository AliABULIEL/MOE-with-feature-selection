"""
OLMoE-1B-7B-0924 MoE Analysis Pipeline
====================================

Complete end-to-end pipeline for OLMoE-1B-7B-0924 MoE router analysis.
Combines internal routing logging + KDE training + visualization.

This script uses the existing RouterLogger from moe_internal_logging.py

Usage:
    python run_olmoe_pipeline.py
...
def load_model(config: Dict) -> Tuple[Any, Any]:
    """Load OLMoE-1B-7B-0924 model and tokenizer."""
...
def run_evaluation(
    ...
    """Run evaluation using RouterLogger from moe_internal_logging.py."""

    print(f"\n{'='*60}\nEvaluating on {dataset_name.upper()}\n{'='*60}")

    # Use the imported RouterLogger from moe_internal_logging.py
                ...
                sample_data["layers"].append(
                    {
                        ...
                        "selected_experts": layer_data["expert_indices"]
                        .numpy()
                        .tolist(),
...
def train_kde_models(
    layers_logits: np.ndarray, output_dir: Path, model_name: str, config: Dict
):
    print(f"\n{'='*60}\nTRAINING KDE MODELS\n{'='*60}")
    ...
    for layer_idx in tqdm(range(layers_logits.shape[0]), desc="Training KDE"):
        ...
        with open(
            output_dir / f"{model_name}_distribution_model_layer_{layer_idx}.pkl", "wb"
        ) as f:
        ...
        for trim_amt in config["kde_trim_amounts"]:
            if trim_amt == 0:
                continue
            trimmed = trim_top_and_bottom_experts(
                layers_logits[layer_idx][np.newaxis, :, :], trim_amt
            ).flatten()
            kde_t = gaussian_kde(trimmed)
            x_t = np.linspace(
                trimmed.min() - 0.2 * abs(trimmed.min()),
                trimmed.max() + 0.2 * abs(trimmed.max()),
                10000,
            )
        ...
        for kernel in config["kde_kernels"]:
            ...
            with open(
                output_dir
                / f"{model_name}_distribution_model_layer_{layer_idx}_{kernel}.pkl",
                "wb",
            ) as f:
...
def generate_basic_plots(
    data, layers_logits, layers_weights, layers_choices, output_dir, model_name, config
):
    print(f"\n{'='*60}\nGENERATING BASIC PLOTS\n{'='*60}")

    num_layers, num_experts = layers_logits.shape[0], layers_logits.shape[2]
    ...
    for layer_idx in tqdm(range(num_layers), desc="Weight sum plots"):
        ...
        save_plot(
            ...
            output_dir
            / "expert_weights_sum"
            / f"{model_name}_layer_{layer_idx}_weights_sum.png",
            config,
        )
    ...
    for layer_idx in tqdm(range(num_layers), desc="Choice count plots"):
        ...
        sns.barplot(
            ...
            y=[choice_dict.get(e, 0) for e in range(num_experts)],
            ax=ax,
        )
        ...
        save_plot(
            ...
            / f"{model_name}_layer_{layer_idx}_choice_counts.png",
            config,
        )
    ...
    for layer_idx in tqdm(range(num_layers), desc="Distribution plots"):
        ...
        save_plot(
            ...
            output_dir
            / "router_softmax"
            / f"{model_name}_layer_{layer_idx}_softmax.png",
            config,
        )
        ...
        save_plot(
            ...
            output_dir
            / "router_raw_logits"
            / f"{model_name}_layer_{layer_idx}_raw_logits.png",
            config,
        )
...
def generate_kde_plots(
    ...
    for layer_idx in tqdm(range(num_layers), desc="KDE plots"):
        with open(
            kde_dir / f"{model_name}_distribution_model_layer_{layer_idx}.pkl", "rb"
        ) as f:
            kde_data = pickle.load(f)

        probs = np.interp(
            test_logits[layer_idx].flatten(), kde_data["x"], kde_data["cdf"]
        )
        ...
        save_plot(
            ...
            / f"{model_name}_kde_density_layer_{layer_idx}_{test_ds}.png",
            config,
        )
        ...
        for i, trim_amt in enumerate(config["kde_trim_amounts"]):
            ...
            if trim_amt == 0:
                train_t, test_t = (
                    train_logits[layer_idx].flatten(),
                    test_logits[layer_idx].flatten(),
                )
            else:
                train_t = trim_top_and_bottom_experts(
                    train_logits[layer_idx][np.newaxis, :, :], trim_amt
                ).flatten()
                test_t = trim_top_and_bottom_experts(
        ...
        save_plot(
            ...
            output_dir
            / "trimmed_comparison"
            / f"{model_name}_trimmed_layer_{layer_idx}.png",
            config,
        )
        ...
        for kernel in config["kde_kernels"]:
            ...
            if kpath.exists():
                with open(kpath, "rb") as f:
                    kdata = pickle.load(f)
                kprobs = np.interp(
                    test_logits[layer_idx].flatten(), kdata["x"], kdata["cdf"]
                )
                sns.histplot(
                    kprobs,
                    bins=100,
        ...
        save_plot(
            ...
            output_dir
            / "kernel_comparison"
            / f"{model_name}_kernel_layer_{layer_idx}.png",
            config,
        )
...
def generate_pvalue_plots(
    ...
    num_layers, num_experts = train_logits.shape[0], train_logits.shape[2]
    ...
    for layer_idx in tqdm(range(num_layers), desc="P-value plots"):
        ...
        save_plot(
            ...
            output_dir
            / "pvalue_basic"
            / f"{model_name}_pvalue_layer_{layer_idx}_{test_ds}.png",
            config,
        )
        ...
        for j in range(0, num_experts, 4):
            ...
            save_plot(
                ...
                output_dir
                / "pvalue_vs_uniform"
                / f"{model_name}_pvalue_uniform_layer_{layer_idx}_exp_{j+1}_to_{min(j+4, num_experts)}.png",
                config,
            )
        ...
        for j in range(0, num_experts, 4):
            ...
            for i, ax in enumerate(axes.flatten()):
                ...
                sns.histplot(
                    exp_pval, bins=50, color="blue", label="KDE P-Value", ax=ax
                )
                ax.set_title(
                    f"{rank+1}-th Popular Expert ({expert_idx}) - Layer {layer_idx}"
                )
                ax.set_xlim(0, 1)
            plt.tight_layout()
            save_plot(
                ...
                / f"{model_name}_pvalue_pop_layer_{layer_idx}_rank_{j+1}_to_{min(j+4, num_experts)}.png",
                config,
            )
...
def run_pipeline(config: Dict):
    print("=" * 70)
    print(f"OLMOE-1B-7B-0924 MOE ANALYSIS PIPELINE")
    ...
    if config["run_evaluation"]:
        print(
            "\n"
            + "=" * 70
            + "\nSTAGE 1: EVALUATION (using RouterLogger from moe_internal_logging.py)\n"
            + "=" * 70
        )
    ...
    if config["run_basic_plots"] and train_ds in routing_data:
        layers_choice_counts = generate_basic_plots(
            routing_data[train_ds]["data"],
            routing_data[train_ds]["logits"],
            routing_data[train_ds]["weights"],
            routing_data[train_ds]["choices"],
            dirs["plots_basic"],
            config["model_name"],
            config,
        )
...
def main():
    """Main entry point."""
    if len(sys.argv) != 2:
        print("Usage: python run_olmoe_pipeline.py <config_file.json>")
        sys.exit(1)
