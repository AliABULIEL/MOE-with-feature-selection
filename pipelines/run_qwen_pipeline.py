...
class QwenRouterLogger:
    """
    Captures internal router outputs using forward hooks for Qwen3-30B-A3B.
    
    Qwen3 MoE architecture:
    - Router gate is at: model.layers[i].mlp (MoE block)
    - The gate submodule computes logits: mlp.gate(hidden_states)
    - Hidden states need reshaping: [batch, seq, hidden] -> [batch*seq, hidden]
    """
    ...
    def register_hooks(self):
        ...
        def create_hook(layer_idx: int):
            def hook_fn(module, input, output):
                """Capture router logits from MoE block."""
                try:
                    with torch.no_grad():
                        hidden_states = input[0]
                        
                        # Qwen MoE: hidden_states is [batch, seq, hidden_dim]
                        # Need to reshape and call gate
                        ...
                        # Compute router logits using the gate submodule
                        if hasattr(module, 'gate'):
                            router_logits = module.gate(hidden_flat.to(torch.float32))
                        else:
                            # Fallback: try to extract from output if available
                            print(f"Warning: Layer {layer_idx} has no 'gate' attribute")
                            return
        ...
        # Register hooks on each MoE layer's MLP block
        num_hooks = 0
        for i, layer in enumerate(self.model.model.layers):
            if hasattr(layer, 'mlp'):
                mlp = layer.mlp
                # Check if this is an MoE layer (has gate attribute)
                if hasattr(mlp, 'gate'):
                    hook = mlp.register_forward_hook(create_hook(i))
                    self.hooks.append(hook)
                    num_hooks += 1
        
        print(f"✅ Registered {num_hooks} router hooks for Qwen3-30B-A3B")
        return num_hooks
...
def generate_basic_plots(
    ...
    for layer_idx in tqdm(range(num_layers), desc="Choice count plots"):
        ...
        fig, ax = plt.subplots(figsize=(24, 6))  # Wider for 128 experts
        ...
        ax.set_ylabel("Count")
        # Reduce x-tick density for 128 experts
...
def generate_per_expert_plots(
    ...
    print(f"Generating {total_plots} plots (this may take a while for 128 experts)...")
...
def generate_pvalue_plots(
    ...
    for layer_idx in tqdm(range(num_layers), desc="P-value uniform plots"):
        ...
        # Plot 4 experts at a time (more iterations for 128 experts)
...
def run_pipeline(config: Dict):
    ...
    if config["run_evaluation"]:
        ...
        logger = QwenRouterLogger(model, config)
