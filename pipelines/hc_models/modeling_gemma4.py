...
from transformers.auto.modeling_auto import AutoModel
...
class Gemma4TextRouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if hc_enabled and getattr(self, "layer_idx", -1) == hc_layer and hc_estimator is not None:
            ...
            # Convert boolean mask to indices and weights
            # To keep Gemma 4 interface, top_k_index and top_k_weights need to be aligned.
            # But Gemma 4's topk returns fixed shape [B*S, K]. 
            # If we enable all experts, K becomes num_experts for those tokens.
            # However, Gemma 4 expects `top_k_index` and `top_k_weights` of shape [B*S, K_max_active].
            # Actually, `Gemma4TextExperts` can handle variable lengths if we pad or just construct it correctly.
            # Let's check `Gemma4TextExperts.forward`. It uses:
            # expert_mask = torch.nn.functional.one_hot(top_k_index, num_classes=self.num_experts)
            # This requires top_k_index to be valid indices. If we pad with `self.num_experts` (which is ignored),
            # it might work.
            
            # Find the max number of active experts for any token
            num_active_experts = expert_mask_bool.sum(dim=-1)
            max_active = num_active_experts.max().item()

            top_k_index = torch.full((expert_mask_bool.size(0), max_active), self.config.num_experts, device=expert_scores.device, dtype=torch.long)
        ...
        # Need to handle the padding index (self.config.num_experts) properly
        # We can append a 1.0 to per_expert_scale or just mask it
